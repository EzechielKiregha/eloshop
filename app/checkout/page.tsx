...
export default function CheckoutPage() {
  ...
  const onSubmit = async (data: CheckoutForm) => {
    ...
    try {
      ...
      setSuccess(true);
      clearCart();
      toast({ title: "Commande créée", description: `N° ${order.orderNumber}`, variant: "default" });
    } catch {
    ...
    return (
      <main>
        ...
        <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
          <EmptyState icon={ShoppingBag} title="Votre panier est vide" description="Ajoutez des articles avant de passer commande.">
            <Button asChild>
      ...
      <main>
        ...
        <div className="mx-auto max-w-lg px-4 py-20 text-center sm:px-6">
          ...
          <p className="mt-3 text-zinc-500 dark:text-zinc-400">
            Votre commande <span className="font-semibold text-zinc-900 dark:text-zinc-50">{orderNumber}</span> a été enregistrée.
          </p>
    ...
    <main>
      ...
      <section className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
        <Button variant="ghost" size="sm" asChild className="mb-4">
          <Link href="/cart"><ArrowLeft className="mr-1 h-4 w-4" />Retour au panier</Link>
        </Button>
        <h1 className="text-3xl font-semibold tracking-tight">Finaliser la commande</h1>
        <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Paiement invité — pas de compte requis</p>
            ...
            <CardContent>
              <form id="checkout-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="address" {...register("address")} placeholder="123 Rue Exemple, Kinshasa" />
